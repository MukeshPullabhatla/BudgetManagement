...
router.post('/login', async (req, res) => {
	...
	let RequestedUser = await userDB.getUser(
		req.body.Username,
		req.body.Password
	);
	if (RequestedUser) {
...
router.post('/register', async (req, res) => {
	...
	if (
		...
		JSON.stringify(Object.keys(req.body)) === JSON.stringify(userObject)
	) {
		let RequestedUser = await new userDB({
...
router.post('/savedconnections', async (req, res) => {
	...
	let UserProfile = await sessionStorage.setUserConnections(
		req.body.RSVP,
		tempConnections,
		req.body.UserID
	);
...
router.post('/connection/delete', async (req, res) => {
	...
	let sendUser = new userModel.User(
		User.id,
		User.UserID,
		User.Email,
		User.FirstName,
	...
	res
		.status(200)
		.send({ User: sendUser, Profile: UserProfile, feedback: responseFeedback });
});
